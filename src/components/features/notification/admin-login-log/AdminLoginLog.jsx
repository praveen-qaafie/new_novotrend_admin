...
import { useAdminLoginLogsQuery } from "@/services/notification/notification.query";
...
const getRecords = data => {
  ...
  if (Array.isArray(response?.admin_logs)) return response.admin_logs;
  if (Array.isArray(response?.admin_login_logs)) return response.admin_login_logs;
...
export default function AdminLoginLogs() {
  ...
  const { data, isLoading, error } = useAdminLoginLogsQuery({
    limit,
    offset,
    search: debouncedSearch,
  });

  const adminLoginLogs = getRecords(data);
  const total = getTotalRecords(data, adminLoginLogs);
    ...
    <div className="overflow-hidden rounded-3xl border border-border bg-background">
      <div className="overflow-x-auto">
        <TableWrapper
          ...
          description="Track admin login activity and request information"
          actions={
            <>
              <TableSearch
                value={search}
                ...
                placeholder="Search admin login logs"
              />
              <ExportDropdown />
            </>
          }
          ...
          <DataTable headers={tableHeaders}>
            ...
            {error && (
              <TableRow>
                ...
                >
                  {error?.message || "Failed to load admin login logs."}
                </TableCell>
              </TableRow>
            )}

            {!isLoading && !error && adminLoginLogs.length === 0 && (
              <TableRow>
                <TableCell
                  colSpan={tableHeaders.length}
                  className="px-6 py-8 text-center text-sm text-muted-foreground"
            ...
            {!isLoading &&
              !error &&
              adminLoginLogs.map((log, index) => {
                const date = getField(log, ["date", "login_date", "created_at", "time"]);
                const username = getField(log, ["username", "login", "name", "admin_name"]);
                ...
                return (
                  <TableRow
                    key={`${log?.id || log?.log_id || index}-${index}`}
