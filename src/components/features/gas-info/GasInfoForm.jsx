...
import { useAddGasInfoMutation } from "@/services/gas-info/gasInfo.mutation";
import { useGasInfoQuery } from "@/services/gas-info/gasInfo.query";

const getGasInfoData = data => {
  const gasInfo =
    data?.response?.gas_info ||
    data?.response?.gasInfo ||
    data?.response ||
    data?.data?.response ||
    data?.data ||
    data;

  if (Array.isArray(gasInfo)) {
    return gasInfo[0] || {};
  }
...
export default function GasInfoForm({ chain }) {
  const [address, setAddress] = useState("");
  const [privateKey, setPrivateKey] = useState("");
  const [showCurrentPrivateKey, setShowCurrentPrivateKey] = useState(false);
  const [showPrivateKeyInput, setShowPrivateKeyInput] = useState(false);
  const queryClient = useQueryClient();
  const { mutate: addGasInfo, isPending } = useAddGasInfoMutation();
  const { data, isLoading, isError } = useGasInfoQuery({
    type_chain: chain,
  });
  ...
  const currentAddressValue = getAddress(gasInfo);
  const isSameGasInfo =
    trimmedAddress === currentAddressValue && trimmedPrivateKey === privateKeyValue;
  const isSubmitDisabled = isPending || !trimmedAddress || !trimmedPrivateKey || isSameGasInfo;
  ...
  const handleSubmit = event => {
    ...
    addGasInfo(
      ...
      {
        onSuccess: response => {
          toast.success(response?.result || "Gas info added successfully");
          setAddress("");
          setPrivateKey("");
          queryClient.invalidateQueries({ queryKey: ["gas-info", chain] });
        },
        onError: error => {
          toast.error(error?.message || "Unable to add gas info");
        },
      }
    );
  };
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <FormSection
        title={`Add Gas ${chain} Information`}
      ...
      >
        ...
        <form onSubmit={handleSubmit} autoComplete="off">
          <div className="grid gap-6">
            <FormInput
              label="Enter Address"
              name={`gas-${chain.toLowerCase()}-wallet-address`}
            ...
            <FormInput
              label="Private Key"
              name={`gas-${chain.toLowerCase()}-private-key`}
              autoComplete="new-password"
